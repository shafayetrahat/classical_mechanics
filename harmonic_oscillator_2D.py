...
def calculate_lin_momentum(mass, v1_x, v2_x, v1_y, v2_y):
    """Calculate the linear momentum in both dimesions."""
    momentum_x = mass*(v1_x+v2_x)
    momentum_y = mass*(v1_y+v2_y)
...
def calculate_ang_momentum(mass, x1, x2, y1, y2, v1_x, v2_x, v1_y, v2_y):
    ...
    angular_1 = mass * (x1*v1_y - y1*v1_x)
    angular_2 = mass * (x2*v2_y - y2*v2_x)
    return angular_1 + angular_2
...
def leapfrog(timesteps, dt, mass, spring_constant, C, x1, x2, y1, y2, v1_x, v2_x, v1_y, v2_y):
    ...
    y1_list, y2_list = [y1], [y2]
    lin_momentum_list = [calculate_lin_momentum(mass, v1_x, v2_x, v1_y, v2_y)]
    ang_momentum_list = [calculate_ang_momentum(mass, x1, x2, y1, y2, v1_x, v2_x, v1_y, v2_y)]
    ...
    return x1_list, x2_list, y1_list, y2_list, lin_momentum_list, ang_momentum_list

def plot_results(timesteps, x1_list, x2_list, y1_list, y2_list, lin_momentum_list, ang_momentum_list):
    ...
    plt.plot(range(timesteps + 1), lin_momentum_array[:, 0], label="Momentum X", color="red")
    plt.plot(range(timesteps + 1), lin_momentum_array[:, 1], label="Momentum Y", color="blue")
    plt.xlabel("Timestep")
    plt.ylabel("Momentum")
    ...
    for ax, ylim in zip(axes, ylims):
        ax.plot(range(timesteps + 1), ang_momentum_array, label="Angular momentum", color="red")
        ax.grid()
...
if __name__ == "__main__":
    ...
    # Run simulation
    x1_list, x2_list, y1_list, y2_list, lin_momentum_list, ang_momentum_list = leapfrog(
    ...
    # Plot results
    plot_results(timesteps, x1_list, x2_list, y1_list, y2_list, lin_momentum_list, ang_momentum_list)
